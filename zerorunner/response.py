...
from .exceptions import ParamsError, ValidationFailure
from .parse import Parser
from requests import Response
from loguru import logger
from zerorunner import exceptions
from zerorunner.parse import parse_data, parse_string_value
import jmespath
...
class ResponseObjectBase(object):
    ...
    def extract(self,
                extractors: typing.List[models.ExtractData],
                variables_mapping: models.VariablesMapping = None,
                functions_mapping: models.FunctionsMapping = None,
    ...
    def _search_jsonpath(self, expr: models.ExtractData) -> typing.Any:
        try:
    ...
    def validate(
        ...
        for v in validators:
            ...
            if check_mode == models.CheckModeEnum.jmespath.value:
                check_value = self._search_jmespath(check_item)
            elif check_mode == models.CheckModeEnum.JsonPath.value:
                expr = models.ExtractData(**u_validator)
                expr.path = check_item
                check_value = self._search_jsonpath(expr)
            elif check_mode == models.CheckModeEnum.variable_or_func.value:
            ...
            #     check_value = check_item
            elif check_mode == models.CheckModeEnum.ResponseHeaders.value:
                check_item = self.resp_obj.headers.get(check_item, None)
                check_value = check_item
            else:
