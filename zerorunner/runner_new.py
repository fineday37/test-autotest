...
from zerorunner.models import TStepResultStatusEnum, VariablesMapping, FunctionsMapping, TStepControllerDict, \
    TStepLogType
from zerorunner.models import StepResult, TestCaseSummary, TestCaseInOut
from zerorunner.models import TStep, TConfig
...
class SessionRunner(object):
    ...
    def run_step(self, step, step_tag: str = None, parent_step_result: StepResult = None):
        ...
        for i in range(step.retry_times + 1):
            try:
                print([m for m in dir(step) if callable(getattr(step, m))])
                print(type(step))
                ...
                if i == step.retry_times:
                    ...
                    logger.warning(
                        f"运行步骤 {step.name()} 校验失败,等待 {step.retry_interval} 秒后重试a"
                    )
