    ...
    def _fetch(self, query: str, size: int = -1, commit: bool = True):
        ...
        if query.upper()[:6] == "SELECT":
            if size < 0:
                ...
                on = dict(zip(result.keys(), result.fetchone()))
                self.value_decode(on)
                return on or None
            else:
                mny = result.fetchmany(size)
                mny = [dict(zip(result.keys(), el)) for el in mny]
                for el in mny:
                    self.value_decode(el)
                return mny or None
...
if __name__ == "__main__":
    ...
    db = DBEngine("mysql+pymysql://root:kzg1814666@47.92.76.123/test-backend")
    # print(db.fetchmany("""
    # select* from student""", 5))
    print(db.fetchmany("select* from student", 2))
