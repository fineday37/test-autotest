...
from zerorunner.report_test import HtmlTestResult
...
from zerorunner.report import stringify
...
class ZeroRunner(object):
    ...
    def add_tests(self, testcase: TestCase) -> typing.List[unittest.TestSuite]:
        ...
        for index, step in enumerate(testcase.teststeps):
            test_runner = self.session_runner if self.session_runner else SessionRunner().with_config(testcase.config)
            # test_runner = SessionRunner().with_config(testcase.config)
    ...
    def run_tests(self, testcase: TestCase):
        ...
        stringify.stringify_summary(self._summary)
    ...
    def _aggregate(self, tests_results):
        ...
        summary = models.TestCaseSummary(name="", success=True, case_id=None)

        for index, tests_result in enumerate(tests_results):
            testcase, result = tests_result
            testcase_summary: models.TestCaseSummary = result.summary
            if index == 0:
