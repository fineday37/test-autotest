...
from zerorunner.models import TStep, StepResult, TStepLogType
import copy
from zerorunner.ext.uploader import prepare_upload_step
from zerorunner.parse import parse_variables_mapping, Parser
from zerorunner.models import Hooks, VariablesMapping
from zerorunner.script_code import Zero
from zerorunner.larder import load_script_content
from zerorunner.parse import build_url
from zerorunner.response import ResponseObject
from zerorunner import utils, models, exceptions
from zerorunner.parse import parse_data, get_mapping_function, \
    Parser
...
def run_api_test(runner: SessionRunner, step: TStep, step_tag: str = None):
    resp = requests.session().request("get", "http://localhost:9000/user/code")
    print(resp.text)
...
def run_api_request(runner: SessionRunner,
    ...
    try:
        ...
        # 获取变量并合并
        parsed_request_dict = Parser().parse_data(
        ...
        request_dict["verify"] = runner.config.verify
        parsed_request_dict["json"] = parsed_request_dict.pop("req_json", {})
        # parsed_request_dict 中存在重复变量，记得删除或者只用
        del parsed_request_dict["method"]
        del parsed_request_dict["url"]
        # 更新会话请求头
        ...
        # request
        # print(request_dict)
        # res = resp = requests.session().request(method, url_path)
        # print(res.json())
        ...
        try:
            ...
            runner.set_step_result_status(step_result=step_result, status=models.TStepResultStatusEnum.success)
        except exceptions.ValidationFailure as err:
            session_success = False
            runner.set_step_result_status(step_result, models.TStepResultStatusEnum.fail, str(err))
            log_req_resp_details()
            # log testcase duration before raise ValidationFailure
            raise
    except exceptions.MyBaseError as err:
        runner.set_step_result_status(step_result, models.TStepResultStatusEnum.err, str(err))
        raise
