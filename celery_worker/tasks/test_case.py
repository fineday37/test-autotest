...
from corelibs.consts import TEST_EXECUTE_SET, TEST_EXECUTE_STATS, CACHE_WEEK, TEST_EXECUTE_TASK
from db.redis import MyRedis
from models.api_models import ApiCase, ApiTestReport
from schemas.api.api_case import TestCaseRun
from schemas.api.api_info import ApiRunSchema
from schemas.api.api_report import TestReportSaveSchema
from schemas.api.timed_task import TaskKwargsIn
...
async def async_run_testcase(case_id: typing.Union[str, int], report_id: [str, int] = None, **kwargs: typing.Any):
    ...
    if not case_id:
        ...
        return
    logger.info(f"开始执行用例: {case_info}")
    ...
    if case_info.step_rely:  # 步骤依赖:
        ...
        summary_params = TestReportSaveSchema(**report_params.model_dump())
        ...
        for step in testcase.teststeps:
            ...
            await r.cus_lpush(testcase_list_key, new_testcase.model_dump(by_alias=True))
        # 设置默认统计数据
