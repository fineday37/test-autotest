...
from corelibs.pagination import parse_pagination
...
from utils.des import encrypt_rsa_password
from corelibs.serialize import unwrap_scalars
...
class Base:
    ...
    @classmethod
    async def get_filter_all(cls, params: typing.List[typing.Any]):
        """
        筛选数据
        :param params:
        :return:
        """
        stmt = select(cls.get_table_columns()).where(*params)
        return await cls.get_result(stmt)
    ...
    @classmethod
    async def update(cls, ids: int, params: typing.Dict):
        stmt = update(cls).where(cls.id == ids).values(**params)
    ...
    async def handle_params(cls, params: typing.Any) -> typing.Any:
        ...
        if isinstance(params, dict):
            ...
            try:
                # 判断是否存在当前用户，存在则使用该用户的id作为修改或者新增id
